...
public class OASRefExtractTest {
    ...
    public void resolvesAbsoluteRelativeAndRejectsNonHttp() {
        // absolute passes through (fragment stripped)
        assertEquals("http://h/a.yaml",
                OASParserUtil.resolveToHttpUrl("http://h/a.yaml#/X", null));
        // relative resolves against a remote base
        ...
        // relative without a remote base is unfollowable
        assertNull(OASParserUtil.resolveToHttpUrl("./b.yaml", null));
        // local base keeps relative refs local (not remote)
        assertNull(OASParserUtil.resolveToHttpUrl("./b.yaml", "file:/tmp/main.yaml"));
        // pure fragment / non-http scheme rejected
