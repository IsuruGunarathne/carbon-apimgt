...
final class RedirectSafeXsdFetcher {
    ...
    static Result fetch(String url, RemoteUrlValidator policy) throws IOException {
        ...
        for (int hop = 0; hop <= MAX_REDIRECTS; hop++) {
            assertAllowed(current, policy);                 // re-validate BEFORE every request / hop
            HttpURLConnection conn = (HttpURLConnection) new URL(current).openConnection();
            conn.setInstanceFollowRedirects(false);         // the key: never auto-follow a redirect
            ...
            try {
                ...
                if (code >= 300 && code < 400) {
                    ...
                    current = resolveLocation(current, location);   // resolve relative redirects, then re-loop
                    continue;
                }
