 ...
 */
public class XmlSchemaSsrfResolverIntegrationTest {
    ...
    public static void startServer() throws IOException {
        // Force the JDK's built-in SchemaFactory to match the production environment.
                ...
                switch (path) {
                    case "/main.xsd":
                        content = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                                ...
                                "            targetNamespace=\"urn:ssrf:main\"\n" +
                                "            xmlns:imp=\"urn:ssrf:imported\">\n" +
                    ...
                    case "/imported.xsd":
                        content = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                                "<xsd:schema xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\"\n" +
                                "            targetNamespace=\"urn:ssrf:imported\">\n" +
    ...
    // -----------------------------------------------------------------------
    // Helper: build SchemaFactory with EXACT production wiring and call newSchema
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Test 1: resolver is consulted for xsd:import AND fetch happens when allowed
    // -----------------------------------------------------------------------
    ...
    public void testImportResolverConsultedAndFetchedWhenAllowed() throws Exception {
        ...
        // Record-only policy: never throws, just logs what the resolver saw
        RemoteUrlValidator recordOnly = validatedUrls::add;

        newSchema("main.xsd", recordOnly);

        // The resolver must have been called with the imported schema URL
        assertTrue(
        ...
        // The HTTP server must have actually served /imported.xsd
        assertTrue(
    ...
    // -----------------------------------------------------------------------
    // Test 2: rejecting in the resolver prevents the import from being fetched
    // -----------------------------------------------------------------------
    ...
    public void testImportBlockedPreventsFetch() throws Exception {
        ...
        // Block policy: throws on any URL containing "imported.xsd"
        RemoteUrlValidator blockImported = url -> {
            if (url.contains("imported.xsd")) {
                throw new APIManagementException("SSRF block: " + url);
            }
        };
        ...
        // The HTTP server must NOT have fetched /imported.xsd (blocked before fetch)
        assertFalse(
    ...
    // -----------------------------------------------------------------------
    // Test 3 (key empirical question): is the resolver consulted for external DTDs?
    //
    // Empirical finding (2026-06-17, JDK 21 / com.sun.org.apache.xerces.internal):
    //   The LSResourceResolver IS consulted for external DTDs.  Xerces calls
    //   resolveResource() with type "http://www.w3.org/TR/REC-xml" (not the XSD
    //   namespace).  When the resolver throws XsdRefBlockedException, newSchema()
    //   surfaces it (directly or wrapped in a SAXException/RuntimeException), and
    //   the HTTP server is NOT contacted for the DTD URL.
    //
    // DESIRED secure outcome: /evil.dtd is NOT fetched (resolver covered the DTD
    // and blocked it).
    //
    // If this test ever starts failing (/evil.dtd IS fetched) it would mean the
    // underlying JAXP/Xerces impl changed and the LSResourceResolver is no longer
    // consulted for external DTDs — a real SSRF gap. The fallback in that case is
    // to set ACCESS_EXTERNAL_DTD="" in XMLSchemaValidator.validateSchema() to block
    // all external DTDs outright (external DTDs in an XSD are never legitimate).
    // -----------------------------------------------------------------------
    ...
    public void testExternalDtdBehavior() throws Exception {
        ...
        // Block policy: throws on any URL containing "evil.dtd"
        ...
        // ASSERT THE SECURE OUTCOME: /evil.dtd must NOT have been fetched.
        // On JDK 21's built-in Xerces the resolver IS consulted for the external DTD
        // (with type="http://www.w3.org/TR/REC-xml"), so the block takes effect and
        // the HTTP server never receives the request.
        // If this assertion fails, the resolver gap fallback (ACCESS_EXTERNAL_DTD="")
        // must be applied to XMLSchemaValidator.validateSchema().
        assertFalse(
                "SSRF GAP DETECTED: /evil.dtd was fetched even though the resolver should have " +
        ...
        // newSchema must have thrown because the block propagated out
        assertNotNull("newSchema should have thrown for the DTD-bearing XSD", caught);

        // The block must be findable in the cause chain
        assertNotNull(
