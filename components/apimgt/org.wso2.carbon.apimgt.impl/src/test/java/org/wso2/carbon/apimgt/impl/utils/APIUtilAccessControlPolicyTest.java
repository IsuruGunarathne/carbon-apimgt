 ...
 * These are the heart of the SSRF gate and previously had no direct coverage (only slow
 * integration suites / tests that mock {@code APIUtil} away).
 *
...
public class APIUtilAccessControlPolicyTest {
    ...
    private static final String PUBLIC_IP = "93.184.216.34";   // literal public IP (example.com), no DNS
    ...
    public void allowMode_emptyHosts_failsClosed() throws Exception {
        // allow mode with no hosts must block everything (fail-closed), regardless of blockPrivate
    ...
    public void allowMode_wildcardHostnameMatch_isAllowedWithoutDns() throws Exception {
        // hostname match short-circuits before any DNS resolution -> deterministic
    ...
    public void allowMode_ignoresBlockPrivate_listedPrivateIpAllowed() throws Exception {
        // block_private_network_access has NO effect in allow mode: an allow-listed private IP passes
    ...
    public void denyMode_privateIp_blockPrivateOff_isAllowed() throws Exception {
        // proves block_private_network_access is what gates the private-range check in deny mode
    ...
    public void unknownHost_allowMode_failsClosed() throws Exception {
        // hostname does not match the list -> resolution attempted -> UnknownHostException -> blocked
    ...
    public void wildcard_dotIsLiteralNotRegexAny() throws Exception {
        // toWildcardRegex Pattern.quote()s literals, so '.' is literal: "a.b" must NOT match "axb"
    ...
    // These assert the CURRENT (audited, intentionally-deferred) behaviour so the suite is green AND a future
    // fix/regression is flagged. See SSRF_IMPL.md "Known Limitations" + audit findings G6/G7.

    @Test
    public void knownLimitation_G6_ipv6DenyEntryDoesNotMatchExpandedResolvedForm() throws Exception {
        // isAnyResolvedIpInList checks address.getHostAddress(), which for ::1 is the EXPANDED form
        // "0:0:0:0:0:0:0:1"; a natural deny entry "::1" does not match it (literal string compare).
        // Documents G6 (IPv6 literal deny entries are effectively dead in the host list). Deferred.
    ...
    public void knownLimitation_G7_trailingDotFqdnBypassesNameDenyList() throws Exception {
        // "internal.corp." (trailing dot) does not match the anchored regex for "internal.corp".
        // Documents G7 (trailing-dot FQDN bypass of name-based deny patterns). Deferred.
