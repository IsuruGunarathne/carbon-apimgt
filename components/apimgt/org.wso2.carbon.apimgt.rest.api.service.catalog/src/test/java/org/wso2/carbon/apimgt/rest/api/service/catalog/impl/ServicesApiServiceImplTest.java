 ...
 * URL through {@link APIUtil#validateRemoteURL(String, String)} (the SSRF gate) BEFORE the URL is
 * fetched, for both the OpenAPI and AsyncAPI branches. Mirrors the publisher import-by-URL gating.
 */
...
public class ServicesApiServiceImplTest {
    ...
    /**
     * The blocked URL must be rejected by the SSRF gate (APIUtil.validateRemoteURL) BEFORE the
     * OAS definition is fetched from the URL. Asserts the by-URL fetch is never reached and the
     * resulting validation response is invalid (so the caller surfaces a 400).
     */
    @Test
    public void testValidateOpenAPIDefinitionByUrlBlockedBeforeFetch() throws Exception {
        ...
        // The gate fired on the blocked URL ...
        PowerMockito.verifyStatic(APIUtil.class);
        APIUtil.validateRemoteURL(eq(BLOCKED_URL), nullable(String.class));
        // ... and the by-URL fetch must never run.
        ...
        // The response must be invalid so the caller returns a 400.
        assertFalse("Blocked URL must yield an invalid definition response", response.isValid());
    }

    /**
     * Same contract for the AsyncAPI by-URL branch: the SSRF gate must reject the blocked URL
     * before AsyncApiParserUtil.validateAsyncAPISpecificationByURL fetches it.
     */
    @Test
